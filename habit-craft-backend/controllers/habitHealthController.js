...
const getHabitHealthScores = async (req, res) => {
  try {
    ...
    const healthData = await Promise.all(habits.map(async (habit) => {
      ...
      // Fetch UNIQUE completed dates for this specific habit within the 30-day window
      ...
      // The Health Formula: (Unique Actual Completions / Expected Completions) * 100
      let healthScore = 0;
      if (expectedDays > 0) {
