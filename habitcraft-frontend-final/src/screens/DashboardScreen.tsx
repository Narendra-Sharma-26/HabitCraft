...
export default function DashboardScreen({ navigation }: any) {
  ...
  // ⭐ NEW: Gatekeeper to prevent the infinite wipe-and-sync loop
  ...
  const fetchDashboard = async () => {
    try {
      ...
      // ⭐ THE FIX: Only resync notifications ONCE per session, not on every tab switch
      if (response.data.habits && !hasSynced.current) {
          await syncHabitNotifications(response.data.habits);
          hasSynced.current = true; // Lock the gate
      }
  ...
  // [The rest of DashboardScreen.tsx remains unchanged...]
